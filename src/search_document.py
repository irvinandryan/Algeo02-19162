...
#fungsi untuk update master setelah user upload file baru
    ...
    newflist = [p for p in pathlib.Path('../test').iterdir() if (str(p)[-4:] == ".txt" and str(p)[8:-4].replace("_"," ") not in titles)]
    flist.extend(newflist)
    addsearch_size = len(newflist)
    if (addsearch_size > 0):
        print("reloading...")
        search_size += addsearch_size
        titles.extend([str(newflist[i])[8:-4].replace("_"," ") for i in range(addsearch_size)])
...
def setup():
    flist = [p for p in pathlib.Path('../test').iterdir() if (str(p)[-4:] == ".txt")]
    search_size = len(flist) #berapa banyak dokumen yang diproses
    titles = [str(flist[i])[8:-4].replace("_"," ") for i in range(search_size)]
